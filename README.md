...
- 用「主题」入口按公司（豆包、千问、混元、夸克、Kimi、DeepSeek、MiniMax、OpenAI…）、技术方向（Agent、编码、多模态…）或产业透镜（付费订阅、产品入口、Agent 生态、办公组织）浏览跨时间的匹配故事，和栏目 tab 是两层不同的分类
...
主题是**另一层浏览入口**，不是顶部「全部/模型/产品/…」栏目 tab。栏目 tab 切的是当天时间轴里的内容类型；主题页按公司、产品、技术方向或产业透镜，把匹配到的故事收成一张可配置的卡片墙，再点进去看该主题的条目列表（默认精选/合并故事优先，时间倒序）。`/topics/` 顶部可用芯片筛选「按哪一个主题」（全部 / 产业透镜及其四个主题 / 公司与模型 / 技术方向）。

- 前端只读 `data/topics.json` 和 `data/topics/<id>.json`，不内置主题名单。
- 维护者改 `config/topics.json`：`id`、`name`、`description`、`group`、`keywords` / `exclude` / 可选 `patterns`。分组可设 `filter: "topics"`，主题页顶部就会展开成该组的主题芯片。公司主题和产业透镜默认还会扫 `archive.json` 做更长窗口；偏宽的技术主题默认只用 24h 故事池，减少误伤。
