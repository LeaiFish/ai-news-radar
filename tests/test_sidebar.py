...
def test_home_and_classic_load_shared_sidebar():
    for path, root in (("index.html", "./"), ("classic/index.html", "./")):
        source = read(path)
        assert f"{root}assets/sidebar.css?v=pane-1" in source
        assert f"{root}assets/sidebar.js?v=pane-1" in source
...
def test_topic_template_loads_sidebar_with_depth_root():
    ...
    assert "{root}assets/sidebar.css?v=pane-1" in source
    assert "{root}assets/sidebar.js?v=pane-1" in source
