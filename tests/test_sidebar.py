...
def test_sidebar_maps_to_existing_radar_features_not_aihot_products():
    js = read("assets/sidebar.js")
    for label in ("精选", "全部 AI 动态", "热点榜", "AI 日报", "主题", "收藏", "Agent 接入", "关于"):
        assert label in js
    assert "模型榜" not in js
    assert "Tibo" not in js
    assert "skills/radar/README.md" in js
...
def test_home_and_classic_load_shared_sidebar():
    for path, root in (("index.html", "./"), ("classic/index.html", "./")):
        source = read(path)
        assert f"{root}assets/sidebar.css?v=sidebar-1" in source
        assert f"{root}assets/sidebar.js?v=sidebar-1" in source
...
def test_topic_template_loads_sidebar_with_depth_root():
    ...
    assert "{root}assets/sidebar.css?v=sidebar-1" in source
    assert "{root}assets/sidebar.js?v=sidebar-1" in source
...
def test_mobile_and_classic_apps_honor_nav_query():
    for path in ("assets/app.js", "classic/assets/app.js"):
        ...
        assert 'nav === "brief"' in source or 'next === "brief"' in source
